...
//circle
void LBP::getCircularLBPFeatureOptimization(const Mat &src, Mat &dst,int radius,int neighbors)
{
#define _tp float
    //Mat src = _src.getMat();
    //CV_Assert(src.depth() == CV_8U&&src.channels() == 1);
    Mat extendedImage;
    float neighbor;
    int lbpcode = 0;
    //LBP特征图像的行数和列数的计算要准确
    dst.create(src.size(),src.type());
    copyMakeBorder(src, extendedImage, radius, radius, radius, radius, BORDER_DEFAULT);
    //dst.create(src.rows-2*radius,src.cols-2*radius,CV_8UC1);

    //extendedImage;
    //Mat dst = _dst.getMat();
    dst.setTo(0);
    for(int i=radius;i<extendedImage.rows-radius;i++)
    {
        //cout<<"a"<<i<<";";
        for(int j=radius;j<extendedImage.cols-radius;j++)
        {
            //cout<<"b"<<j<<";";
    for(int k=0;k<neighbors;k++)
    {
        ...
        //循环处理每个像素
                ...
                //cout<<"i:";
                //获得中心像素点的灰度值
                _tp center = extendedImage.at<_tp>(i,j);
                //根据双线性插值公式计算第k个采样点的灰度值
                neighbor = extendedImage.at<_tp>(i+x1,j+y1) * w1 + extendedImage.at<_tp>(i+x1,j+y2) *w2 \
                    + extendedImage.at<_tp>(i+x2,j+y1) * w3 +extendedImage.at<_tp>(i+x2,j+y2) *w4;
                //float neighbor = 100.0;
        //cout<<"j:";
...
//LBP_new
void LBP::ComputeLBPImage_new(const Mat &srcImage, Mat &LBPImage)
{
    // 参数检查，内存分配
    ...
    // 扩充原图像边界，便于边界处理
    Mat extendedImage;
    copyMakeBorder(srcImage, extendedImage, 2, 2, 2, 2, BORDER_DEFAULT);

    // 计算LBP特征图
    int heightOfExtendedImage = extendedImage.rows;
    int widthOfExtendedImage = extendedImage.cols;
    int widthOfLBP=LBPImage.cols;
    uchar *rowOfExtendedImage= extendedImage.data+2*widthOfExtendedImage+2;
    uchar *rowOfLBPImage = LBPImage.data;
    uchar *colOfExtendedImage;
    uchar *colOfLBPImage;
    int avgcenter;
    int p1,p2,p3,p4,p5,p6,p7,p8;
    for (int y = 2; y <= heightOfExtendedImage - 3; ++y, rowOfExtendedImage += widthOfExtendedImage, rowOfLBPImage += widthOfLBP)
    {
        // 列
        colOfExtendedImage = rowOfExtendedImage;
        colOfLBPImage = rowOfLBPImage;
        for (int x = 2; x <= widthOfExtendedImage - 3; ++x,++colOfExtendedImage,++colOfLBPImage)
        {

            //avgcenter = (colOfExtendedImage[0 - widthOfExtendedImage - 1]+colOfExtendedImage[0 - widthOfExtendedImage]+colOfExtendedImage[0 - widthOfExtendedImage+1]+colOfExtendedImage[0 + 1]+colOfExtendedImage[0 + widthOfExtendedImage + 1] + colOfExtendedImage[0 + widthOfExtendedImage] + colOfExtendedImage[0 + widthOfExtendedImage - 1] + colOfExtendedImage[0 - 1])/8;
            avgcenter = (colOfExtendedImage[0 - widthOfExtendedImage] + colOfExtendedImage[0 + 1] + colOfExtendedImage[0 + widthOfExtendedImage] + colOfExtendedImage[0 - 1])/4;
            // 计算LBP值
            int LBPValue = 0;
            p1=(colOfExtendedImage[0-widthOfExtendedImage-2]+colOfExtendedImage[0-2*widthOfExtendedImage-1]+colOfExtendedImage[0-widthOfExtendedImage]+colOfExtendedImage[0-1])/4;
            if (p1 >= avgcenter)
                LBPValue += 128;
            p2=(colOfExtendedImage[0-2*widthOfExtendedImage]+colOfExtendedImage[0-widthOfExtendedImage+1]+colOfExtendedImage[0]+colOfExtendedImage[0-widthOfExtendedImage-1])/4;
            if (p2 >= avgcenter)
                LBPValue += 64;
            p3=(colOfExtendedImage[0-2*widthOfExtendedImage+1]+colOfExtendedImage[0-widthOfExtendedImage+2]+colOfExtendedImage[0+1]+colOfExtendedImage[0-widthOfExtendedImage])/4;
            if (p3 >= avgcenter)
                LBPValue += 32;
            p4=(colOfExtendedImage[0-widthOfExtendedImage+1]+colOfExtendedImage[0+2]+colOfExtendedImage[0+widthOfExtendedImage+1]+colOfExtendedImage[0])/4;
            if (p4 >= colOfExtendedImage[0])
                LBPValue += 16;
            p5=(colOfExtendedImage[0+1]+colOfExtendedImage[0+widthOfExtendedImage+2]+colOfExtendedImage[0+2*widthOfExtendedImage+1]+colOfExtendedImage[0+widthOfExtendedImage])/4;
            if (p5 >= avgcenter)
                LBPValue += 8;
            p6=(colOfExtendedImage[0]+colOfExtendedImage[0+widthOfExtendedImage+1]+colOfExtendedImage[0+2*widthOfExtendedImage]+colOfExtendedImage[0+widthOfExtendedImage-1])/4;
            if (p6 >= avgcenter)
                LBPValue += 4;
            p7=(colOfExtendedImage[0-1]+colOfExtendedImage[0+widthOfExtendedImage]+colOfExtendedImage[0+2*widthOfExtendedImage-1]+colOfExtendedImage[0+widthOfExtendedImage-2])/4;
            if (p7 >= avgcenter)
                LBPValue += 2;
            p8=(colOfExtendedImage[0-widthOfExtendedImage-1]+colOfExtendedImage[0]+colOfExtendedImage[0+widthOfExtendedImage-1]+colOfExtendedImage[0-2])/4;
            if (p8 >= avgcenter)
                LBPValue += 1;
...
void LBP::ComputeLBPFeatureVector_256(const Mat &srcImage, Size cellSize, Mat &featureVector)
{
    ...
    //ComputeLBPImage_new(srcImage,LBPImage);
    //getCircularLBPFeatureOptimization(srcImage, LBPImage, 1, 8);
