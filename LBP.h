...
{
public:
    void getCircularLBPFeatureOptimization(const Mat &src, Mat &dst, int radius, int neighbors);

    void ComputeLBPImage_57(const Mat &srcImage, Mat &LBPImage);
    void ComputeLBPFeatureVector_57(const Mat &srcImage,Size cellSize,Mat &featureVector);
    void ComputeLBPImage_new(const Mat &srcImage,Mat &LBPImage);
    // 计算基本的256维LBP特征向量
